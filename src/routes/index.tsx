  ...
  return (
    ...
    >
      <style>{`
        ...
        .text-gradient-gold { color: #FF4B12; }
        .bg-gold-grad { background: #FF4B12; }
        .shadow-gold { box-shadow: 0 8px 30px -4px rgba(255,75,18,.5); }
        ...
        <div className="container mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-gold-grad hover:brightness-110 text-white font-bold px-4 sm:px-6 py-2.5 rounded-full text-xs sm:text-sm uppercase tracking-wide transition-colors shrink-0"
            >
              Diagnóstico grátis
            </button>
            <button
      ...
      <section id="hero" className="relative isolate pt-28 pb-10 sm:pt-36 sm:pb-14 overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 text-center max-w-3xl">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              onClick={scrollToDiagnostico}
              className="w-full sm:w-auto bg-gold-grad hover:brightness-110 text-white font-bold px-8 py-4 rounded-full shadow-gold uppercase tracking-wide text-sm transition-colors"
            >
            ...
            <form onSubmit={onInlineSubmit} className="space-y-4">
              ...
              <div>
                ...
                <DarkSelect name="faixa_investimento" required>
                  ...
                  <option value="Entre R$30.000 e R$50.000">30 e 50 mil</option>
                  <option value="Entre R$50.000 e R$75.000">50 e 75 mil</option>
            ...
            <div className="relative bg-white/5 border-2 border-[#FF4B12] rounded-2xl p-6 sm:p-8 shadow-gold backdrop-blur-sm">
              ...
              <ul className="space-y-3.5">
                {[
                  ...
                  <li key={t} className="flex items-start gap-3">
                    <span className="text-[#FF8A5C] shrink-0 mt-0.5">✓</span>
            ...
            {FAQS.map(([q, a]) => (
              <details key={q} className="bg-white/5 border border-white/10 rounded-xl px-5 backdrop-blur-sm group">
                <summary className="cursor-pointer list-none flex justify-between items-center py-4 font-semibold text-sm sm:text-base text-white">
                  <span>{q}</span>
                  <span className="chev transition-transform text-[#FF8A5C]">▼</span>
                </summary>
          ...
          <div className="grid sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {TESTI.map(([n, r, t]) => (
              <div key={n + r} className="bg-white/5 border border-white/10 rounded-xl p-6 backdrop-blur-sm">
                <div className="text-[#FF8A5C] mb-3">★★★★★</div>
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <label className="block text-xs font-semibold text-white/70 mb-1.5 uppercase tracking-wide">
      {children} <span className="text-[#FF8A5C]">*</span>
    </label>
  );
}
...
function Field({
  ...
  return (
    <div>
      <label className="block text-xs font-semibold text-white/70 mb-1.5 uppercase tracking-wide">
        {label} {required && <span className="text-[#FF8A5C]">*</span>}
      </label>
      <input
        name={name}
        type={type}
        ...
        className="w-full bg-white/5 border border-white/15 text-white placeholder-white/30 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#FF4B12] focus:border-[#FF4B12]"
      />
    </div>
  );
}
...
function DarkSelect({ name, required, children }: { name: string; required?: boolean; children: React.ReactNode }) {
  return (
    <select
      name={name}
      required={required}
      className="w-full bg-white/5 border border-white/15 text-white rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#FF4B12] focus:border-[#FF4B12] [&>option]:bg-[#0A1226] [&>option]:text-white"
    >
      {children}
    </select>
  );
